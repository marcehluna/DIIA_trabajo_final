...
| [`explicación-arquitectura.md`](explicación-arquitectura.md) | **Guía de lectura** — relato del diagrama funcional, capa por capa. |
...
| [`stack-tecnológico.md`](stack-tecnológico.md) | **Stack tecnológico** — tabla Componente / Tecnología / Versión / Rol del perfil productivo (E11 + E13). |

Ambos representan el perfil productivo validado en evaluación: retrieval **E11** (léxico, cupos 2+3+2+1) y respuesta **E13** (prompt v3 en español), sobre una arquitectura de **Generación Aumentada por Recuperación (RAG)** [5].
...
| **2** | Consulta RAG en tiempo real | Cada vez que un usuario analiza una protesta | Orquesta el flujo completo: entrada del relato → recuperación → prompt → LLM → informe. Es el **camino productivo** de la aplicación (`app.py`) [5]. |
...
|----------|----------|----------------------------------|
| **Fuentes PDF** (Call Book, Case Book) | Conservar el material oficial en el formato en que lo publica World Sailing / WS [1][2][3]. | Fue la línea base del proyecto (E0) y sigue disponible bajo perfil `baseline`. Aporta señales TR CALL y precedentes de caso cuando el índice los incluye. |
| **Fuentes estructuradas** (CSV reglas RRS, CSV definiciones) | Representar cada regla y definición como registro con metadatos (número, sección, referencias cruzadas). | El EDA mostró que el PDF del RRS no segmenta bien por regla; el CSV permite **un chunk por norma**, clave para subir recall de reglas (E10→E11) [6]. |
...
| **Interfaz Gradio** (`app.py`) | Capturar relato de protesta y protestado; mostrar informe y consola de actividad. | Interfaz mínima viable para demostrar la PoC sin desarrollar un frontend propio [9]. Mismo backend que usa la evaluación. |
...
| **CorpusRetriever** (léxico, `top_k = 8`) | Rankear chunks por solapamiento de tokens con la consulta y devolver los 8 más relevantes. | Elegido tras E0–E17: simple, reproducible y suficiente con corpus en inglés y consulta en español cuando el índice JSONL aporta las reglas [5]. El híbrido léxico+semántico (E15–E17) [12] queda opt-in, no productivo. |
| **Cupos por tipo** (2 RRS + 3 CALL + 2 CASE + 1 DEF) | Reservar slots del `top_k` para cada familia documental. | Evita que un tipo (p. ej. RRS) monopolice los 8 chunks [6]. Configuración **E11**, validada frente a E10 y al índice *full*. |
...
| **Prompt v3 (ES)** (`prompts.py`, system + user, CoT) | Instruir al modelo: citar normas, razonar y cerrar con `Decisión:` en español. | Referencia **E13**: subió dictamen automático al 60 % manteniendo retrieval de E11. El formato fijo y la estrategia *chain-of-thought* [8] habilitan métricas de citas. |
| **HTTPChatClient** | Abstraer la llamada al modelo vía API HTTP compatible con OpenAI. | Desacopla la PoC del proveedor; en local apunta a Ollama [10], consumiendo el endpoint de chat completions [11]. |
| **Ollama — Qwen 2.5 14B** | Ejecutar el LLM localmente (`/v1/chat`). | Modelo usado en todas las corridas completas del golden set [7][10]; equilibrio calidad/latencia en hardware de desarrollo. |
...
- **Retrieval híbrido** (nota al pie del diagrama): variante léxico+semántica evaluada en E15–E17 [12]; no forma parte del perfil por defecto porque no superó a E11+E13 en F1 ni dictamen.

### Lectura del flujo end-to-end

1. **Offline:** PDF y CSV → ingesta → JSONL en `corpus/processed/` → carga según perfil → índice en memoria.
2. **Consulta:** usuario → Gradio [9] → `ProtestPipeline` → retrieval con cupos → prompt v3 → Ollama [10] → informe estructurado.
...
| **`llm/chat_http_client.py`** | Cliente HTTP compatible OpenAI → Ollama `/v1/chat` [10][11]. | Implementación productiva local. |
...
| Módulo | Objetivo | Por qué está en la arquitectura |
|--------|----------|----------------------------------|
| **`prompts.py`** | Plantillas system/user v3 en español, estrategia CoT [8], normalización de idioma. | Toda la ingeniería de prompt vive en un solo módulo versionable (E13). |
...
**Ollama** [10] expone una API HTTP `/v1/chat` compatible con clientes OpenAI [11]. `HTTPChatClient` envía system + user prompt y recibe el informe en streaming o bloque. Es externo al repositorio: la PoC asume que el daemon corre en `127.0.0.1:11434` con el modelo `qwen2.5:14b-instruct` [7] descargado. Cambiar de proveedor implica solo ajustar URL y modelo en `Settings`, no reescribir `pipeline.py`.
...
[10] Ollama. (s. f.). *Ollama — Run large language models locally*. https://ollama.com/

[11] OpenAI. (s. f.). *Chat Completions API reference*. https://platform.openai.com/docs/api-reference/chat

[12] Karpukhin, V., Oguz, B., Min, S., Wu, L., Edunov, S., Chen, D., & Yih, W. (2020). Dense passage retrieval for open-domain question answering. *Proceedings of EMNLP 2020*, 6769–6781. https://arxiv.org/abs/2004.04906
